	...
	const publishProposal = async () => {
		...
		try {
			...
			await addDoc(collection(db, "proposals"), {
				...
				votingCloseTimestamp: closeDate,
				question: questionRef.current?.value,
				options,
				projectName: selectedProject.projectName,
				projectImage: selectedProject.projectImage,
				projectTicker: selectedProject.projectTicker,
			});
				...
				<div ref={previewProposalWrapperRef} className="fixed top-[5vh] bottom-[5vh] left-[20vw] right-[20vw] overflow-y-scroll scrollbar-hide m-auto bg-white rounded-sm p-6 shadow-lg text-gray-700 z-40">
					...
					<div className="flex py-4 space-x-20">
						...
						<div className="w-2/3 flex space-x-2">
							<img src={selectedProject.projectImage} alt="" className="h-6 w-6" />
							<p className="font-semibold">{capitalizeFirstLetter(selectedProject.projectName)}</p>
						</div>
					</div>
					...
					<div className="flex py-4 space-x-20">
						...
						<p className="w-2/3">{unixToDateTime(closeDate, "utc")}</p>
					</div>
