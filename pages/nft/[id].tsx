...
function Nft({ id, nftUrl, name, description, createdTimestamp, creatorAddress, creatorName, creatorImage, projectId, projectName, views, price, contractAddress, tokenId, tokenStandard, chain, creatorFees }: NftType) {
	const router = useRouter();
	const [graphTimeline, setGraphTimeline] = useState<string>("alltime");
	const [nftsInCollection, setNftsInCollection] = useState<NftType[]>([]);

	useEffect(() => {
		async function getOtherNftsFromCollection() {
			let nfts: NftType[] = [];
			const nftQuery = query(collection(db, "nfts"), orderBy("createdTimestamp", "desc"), limit(5));
		...
		<main className="flex flex-col items-center space-y-6 bg-blue-50 w-full py-8">
			<div className="w-3/4 justify-center grid sm:grid-cols-2 gap-6 grid-flow-row-dense h-fit">
				...
				<div className="flex justify-center">
					<img src={nftUrl} alt={name} className="rounded-md border-4 border-white shadow col-span-1" />
				</div>

				{/* Main Details */}
				<div className="bg-white p-4 rounded-sm shadow col-span-1">
					<div className="flex justify-between items-center">
						<p className="text-blue-500 cursor-pointer hover:underline text-sm">{capitalizeFirstLetter(projectName)}</p>
					...
					<div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 items-center sm:space-x-6 border-b-[1px] pb-4">
						<div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 items-center">
							...
							<div onClick={() => router.push(`/profile/${creatorAddress}`)} className="group flex items-center">
					...
					<div className="flex flex-col items-center space-y-2 sm:space-y-0 sm:flex-row justify-between sm:space-x-12 mt-4">
						<div className="flex space-x-2 items-center">
							...
							<div className="flex items-center space-x-1">
								<img src="/eth_logo.png" alt={chain} className="" />
				...
				<div className="bg-white rounded-sm p-4 text-sm text-gray-500 col-span-1 shadow divide-y-2">
					<div className="pb-4">
						...
						<p>
							Created By{" "}
							<span onClick={() => router.push(`/project/${projectId}`)} className="text-blue-500 cursor-pointer hover:underline ml-2">
			...
			<div className="w-3/4">
				<div className="w-full flex justify-between mb-4">
					...
					<button className="text-gray-600 text-sm hover:text-white hover:bg-blue-600 border-[1px] border-blue-200 rounded-sm px-2 py-1">View collection</button>
				</div>
