...
import { ref, getDownloadURL, uploadString, getStorage, uploadBytes, uploadBytesResumable } from "firebase/storage";
...
function SignupModal() {
	...
	const { authenticate, account, enableWeb3 } = useMoralis();
	...
	const handleImageChange = (e) => {
		if (e.target.files[0]) {
			...
			document.getElementById("showImage").src = URL.createObjectURL(e.target.files[0]);
		}
	};
	...
	const registerUser = async () => {
		...
		try {
			const docRef = await setDoc(doc(db, "users", account), {
				bio: "",
