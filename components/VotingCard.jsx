...
function VotingCard({ proposalId, title, description, question, options, projectId, projectName, projectTicker, projectImage, timeLeft }) {
	...
	useEffect(() => {
		...
		async function checkIfVoteExists() {
			const docSnap = await getDoc(doc(db, "proposals", proposalId, "votes", account));

			if (docSnap.exists()) {
				setSubmittedVoteId(docSnap.data().selectionId);
			} else {
	...
	const submitVote = async (e) => {
		...
		try {
			await setDoc(
				doc(db, "proposals", proposalId, "votes", account),
				{
			...
			await setDoc(
				doc(db, "users", account, "votes", proposalId),
				{
					selectionId: selectedOption,
				},
				{ merge: true }
				...
				<div className={`flex flex-col md:flex-row items-center space-x-4 ${submittedVoteId > -1 ? "col-span-4" : "col-span-5"}`}>
					...
					<div>
						<div className="flex flex-col text-center lg:flex-row justify-center items-center md:space-x-2">
							<h2 className="text-gray-700 font-bold text-lg">{capitalizeFirstLetter(projectName)}</h2>
